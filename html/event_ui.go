...
func newUI(body *Body, w *Wasm) *UI {
	nodes := map[string]elemNode{}
...
func (u *UI) Close() error {
	...
	m := idRoot
	if err := mapIDs(u.body, m idRoot); err != nil {
		log.Fatalf("bad bug: %s", err)
	}

	u.wasm.doc.Body = u.body
	u.nodes = m
	u.wasm.doc.ExecuteDomCalls()
...
func (u *UI) Update(id string, with Element) error {
	u.mu.Lock()
	defer u.mu.Unlock()
	...
	pipe := NewPipeline(context.Background(), &http.Request{}, buff)
	with.Execute(pipe)
	u.validate = append(
		u.validate,
		func() error {
...
func (u *UI) AddTo(id string, with Element) error {
	u.mu.Lock()
	defer u.mu.Unlock()
...
func copyBody(body *Body) *Body {
	return deepcopy.Copy(body).(*Body)
}
*/
